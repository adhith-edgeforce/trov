...
class DeliveryRunner(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('tts_rate',                  145)
        self.declare_parameter('tts_volume',                20.0)
        self.declare_parameter('arrival_announcement',      'Arrived at {route}, please unload')
        self.declare_parameter('return_announcement',       'Arrived at store')
        ...
        self._tts_rate            = self.get_parameter('tts_rate').value
        self._tts_volume          = self.get_parameter('tts_volume').value
        self._arrival_announcement = self.get_parameter('arrival_announcement').value
        self._return_announcement = self.get_parameter('return_announcement').value
        ...
        # ── TTS (voice announcement) worker thread ───────────────────────────
        #
        # Same pattern as the beep loop — one background thread renders and
        # plays queued phrases (via espeak + sox play) for the node's whole
        # lifetime, drained through _tts_say(). Fully offline — no network
        # or cloud dependency of any kind.
        #
        self._espeak_ok      = self._check_espeak()
        self._tts_ok         = self._espeak_ok
        self._tts_queue      = []
        self._tts_lock       = threading.Lock()
        self._tts_new_event  = threading.Event()
        self._tts_stop_event = threading.Event()
        self._tts_thread = threading.Thread(
            target=self._tts_worker_thread, daemon=True
        )
        ...
        self.get_logger().info(
            ...
            f'  Voice announcements : {"OK (espeak+play)" if self._tts_ok else "DISABLED"} '
            f'(rate {self._tts_rate}, vol {self._tts_volume}x)\n'
        )
    ...
    # =========================================================================
    # TTS — persistent worker thread (voice announcements)
    # =========================================================================
    #
    # Fully offline: text is rendered to a temp .wav via the local `espeak`
    # CLI, then played via `sox play` — same proven audio path as the beep
    # sound. No network, no cloud API, no external dependency of any kind.
    #

    def _check_espeak(self) -> bool:
        if shutil.which('espeak') is None:
            self.get_logger().warn(
                '[TTS] "espeak" binary not found on PATH — voice announcements DISABLED. '
                'Install with: sudo apt install espeak'
            )
            return False
        if shutil.which('play') is None:
            self.get_logger().warn(
                '[TTS] "play" (sox) binary not found on PATH — voice announcements DISABLED.'
            )
            return False
        return True
    ...
    def _tts_worker_thread(self):
        """
        Drains queued phrases in FIFO order whenever _tts_new_event is set.
        Each phrase is rendered to a temp wav via `espeak -w`, then played
        with the same `play` command/device used by the beep loop.
        Exits when _tts_stop_event is set.
        """
        if not self._tts_ok:
            self._tts_stop_event.wait()
            return

        # NOTE: tts_volume is passed straight through to sox's `vol` effect
        # as a raw multiplier (e.g. 20.0 = 20x), confirmed working on this
        # hardware — no 0.0-1.0 clamp here.
        vol_fraction = self._tts_volume
        self.get_logger().info('[TTS-thread] Ready')

        while not self._tts_stop_event.is_set():
            if not self._tts_new_event.wait(timeout=0.2):
                continue
            self._tts_new_event.clear()
            if self._tts_stop_event.is_set():
                break

            with self._tts_lock:
                pending = self._tts_queue.copy()
                self._tts_queue.clear()

            for phrase in pending:
                if self._tts_stop_event.is_set():
                    break
                self._speak(phrase, vol_fraction)

        self.get_logger().info('[TTS-thread] Exiting')

    def _speak(self, phrase: str, vol_fraction: float):
        """Render `phrase` with espeak to a temp wav, then play it via sox `play`."""
        wav_path = None
        try:
            fd, wav_path = tempfile.mkstemp(prefix='trov_tts_', suffix='.wav')
            os.close(fd)

            gen = subprocess.run(
                ['espeak', '-s', str(self._tts_rate), '-w', wav_path, phrase],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=10.0
            )
            if gen.returncode != 0 or not os.path.exists(wav_path) \
                    or os.path.getsize(wav_path) == 0:
                self.get_logger().error(
                    f'[TTS] espeak failed to render (rc={gen.returncode}): "{phrase}"'
                )
                return

            self.get_logger().info(f'[TTS] "{phrase}"')
            proc = subprocess.Popen(
                ['play', '-q', wav_path, 'vol', str(vol_fraction)],
            ...
            try:
                ...
                proc.kill()
                self.get_logger().error(f'[TTS] play timed out for: "{phrase}"')
        except Exception as e:
            self.get_logger().error(f'[TTS] speak error (ignored): {e}')
        finally:
    ...
    def _tts_say(self, phrase: str):
        """Queue a phrase to be spoken by the TTS worker thread. No-op if TTS is unavailable."""
        if not self._tts_ok:
            self.get_logger().info(f'[TTS] (disabled) would have said: "{phrase}"')
            return
        with self._tts_lock:
            self._tts_queue.append(phrase)
        self._tts_new_event.set()
    ...
    def _mission_body(self):
        ...
        # Do NOT reorder by nearest waypoint. The nearest-waypoint logic caused
        # the second mission to instantly "arrive" when the robot returned to a
        # position near a mid-route waypoint, wrapping the route backwards.
        # WP0 is always the home/start position — always plan from there.
        ...
        # ── 4. ARRIVED — beacon + beep OFF, voice announcement, then pulse pin32 ─
        with self._state_lock:
        ...
        # ── Voice announcement: "Arrived at {route}, please unload" ─────────
        announcement = self._arrival_announcement.format(route=self._route_name)
        self._tts_say(announcement)
        self.get_logger().info(f'[Mission] Announcement queued: "{announcement}"')
        ...
        # Clear both costmaps — removes ghost obstacles left by the person
        ...
        # ── Beacon + beep OFF, voice announcement: back at store ────────────
        self._mission_signal(False)
        self.get_logger().info('[Mission] ✓ Return complete — back home. Beacon OFF, Beep STOP')
        self._tts_say(self._return_announcement)
        self.get_logger().info(f'[Mission] Announcement queued: "{self._return_announcement}"')
        # _run_mission finally block will also call _mission_signal(False) — harmless no-op by then.
    ...
    def destroy_node(self):
        self.get_logger().info('[DeliveryRunner] Shutting down — GPIOs LOW, beep STOP, TTS STOP')
        self._mission_signal(False)
        self._gpio32_set(False)
        ...
        self._tts_new_event.set()       # unblock the tts thread if it's waiting
