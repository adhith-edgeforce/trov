#!/usr/bin/env python3
"""
Outdoor Waypoint Recorder — Map-Scoped Edition
Package : trov
Place at : trov_ws/src/trov/scripts/outdoor_waypoint_recorder.py
...
from the lidar_localization node's map_path parameter.

YAML structure written to outdoor_waypoints.yaml:
  maps:
    adibatla_outdoor:
      routes:
        patrol_zone_a:
...
═══════════════════════════════════════════════════════════
  ...
  waypoint_recorder/generate_interpolated std_msgs/Empty
    ...
    Z coordinate is looked up from TF at generation time and applied
    to all interpolated waypoints.
...
═══════════════════════════════════════════════════════════
  ...
  map_server_node      Name of the localization node (default: 'lidar_localization')
  map_param_name       Parameter name holding the map path (default: 'map_path')
  map_detect_timeout   Seconds to wait for localization node (default: 10.0)
  map_retry_interval   Seconds between map detection retries if startup
                       failed (default: 5.0)
  output_file          Path to outdoor_waypoints.yaml
"""
...
class OutdoorWaypointRecorder(Node):

    def __init__(self):
        super().__init__('outdoor_waypoint_recorder')
        ...
        # Retries _detect_active_map() every map_retry_interval seconds until
        # it succeeds, then cancels itself. This means the node self-heals if
        # lidar_localization wasn't ready when this node started.
        ...
        self.get_logger().info(
            ...
            f'  Loc node    : /{self._map_server_node} (param: {self._map_param_name})\n'
    ...
    def _detect_active_map(self):
        ...
        The map name is derived from the PCD filename stem:
        ...
        if not response.values:
            self.get_logger().error(
                f'[MapDetect] ✗ /{self._map_server_node} returned no value for '
                f'"{self._map_param_name}". '
                f'Will retry every {self._map_retry_interval}s automatically.'
            )
            return
        ...
        if not map_path:
            self.get_logger().error(
                f'[MapDetect] ✗ "{self._map_param_name}" parameter is empty on '
    ...
    def _start_map_select_cb(self, _msg: Empty):
        ...
        self.get_logger().info(
            ...
            f'  Note: Z will be looked up from TF at generation time.'
        )
        self._publish_status()
    ...
    def _generate_interpolated_cb(self, _msg: Empty):
        ...
        # ── Look up robot Z from TF at generation time ────────────────────────
        # Used for all interpolated waypoints. Outdoor terrain is not perfectly
        # flat so we capture the real Z rather than hardcoding 0.0.
        robot_pose = self._get_robot_xyz()
        if robot_pose is not None:
            z_value = robot_pose[2]
            self.get_logger().info(
                f'[MAP_SELECT] Robot Z from TF: {z_value:.4f} m — '
                f'applying to all interpolated waypoints.'
            )
        else:
            z_value = 0.0
            self.get_logger().warn(
                '[MAP_SELECT] Could not look up robot Z from TF — '
                'falling back to z=0.0. Is localization running?'
            )

        anchors = self._resolve_anchors(robot_pose)
        if anchors is None:
            return

        waypoints = self._interpolate_anchors(anchors, z_value)
        if len(waypoints) < 2:
            self.get_logger().warn(
        ...
        self.get_logger().info(
            ...
            f'(map: "{self.active_map}" route: "{self.route_name}" z: {z_value:.4f})'
        )
        self.pending_points = []
        self.waypoints      = []
        self.mode           = Mode.IDLE
    ...
    def _resolve_anchors(
        ...
        Return the ordered list of anchor (x, y) points to interpolate through.
          1 pending point  → [robot_current_xy, pending_point]
          2+ pending points → pending_points as-is

        robot_pose is passed in (already looked up) so we don't do a second
        TF call here. If we need it for the 1-point case and it's None,
        we log an error and return None.
        """
    ...
    def _interpolate_anchors(
        ...
        z_value is applied uniformly to all waypoints. It is the robot's
        actual Z at generation time, which is a reasonable approximation
        for slightly uneven outdoor terrain.
        ...
        for seg_idx in range(len(anchors) - 1):
            ...
            if seg_len < step:
                if seg_idx == 0:
                    result.append(
                        self._make_wp(len(result), x0, y0, z_value, yaw_deg, quat)
                    )
                continue
            ...
            for i in range(n_steps):
                ...
                py = round(y0 + i * dy, 4)
                result.append(
                    self._make_wp(len(result), px, py, z_value, yaw_deg, quat)
                )
        ...
        if len(anchors) >= 2:
            ...
            result.append(
                self._make_wp(
                    len(result), x_last, y_last, z_value, final_yaw_deg, final_quat
                )
            )
    ...
    @staticmethod
    def _make_wp(index: int, x: float, y: float, z: float,
    ...
    def _get_robot_xyz(self) -> tuple[float, float, float] | None:
        """
        Look up current robot position in map frame.
        Returns (x, y, z) or None if TF lookup fails.
        Z is included so MAP_SELECT waypoints reflect real terrain height.
        """
        try:
    ...
    def _timer_callback(self):
        ...
        z = t.transform.translation.z  # real Z captured from TF outdoors
        ...
        self.get_logger().info(
            f'[WP {wp["index"]:03d}] '
            f'x={x:7.3f} y={y:7.3f} z={z:6.3f} yaw={wp["yaw_deg"]:6.1f} deg '
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info(
            f'[OutdoorWaypointRecorder] Stopped. '
