#!/usr/bin/env python3
"""
Outdoor Waypoint Follower — Map-Scoped Edition
Package : trov
Place at : trov_ws/src/trov/scripts/outdoor_waypoint_follower.py

Follows routes that were recorded by outdoor_waypoint_recorder.py.
Routes are scoped to the currently loaded map: the active map name is
auto-detected from the lidar_localization node's map_path parameter at
startup. Attempting to follow a route that was recorded under a
different map (and NOT the active map) is refused loudly.

YAML structure read from outdoor_waypoints.yaml:
  maps:
    adibatla_outdoor:
      routes:
        patrol_zone_a:
...
═══════════════════════════════════════════════════════════
  PARAMETERS
═══════════════════════════════════════════════════════════
  waypoints_file       Path to outdoor_waypoints.yaml
...
class OutdoorRouteFollower(Node):

    def __init__(self):
        super().__init__('outdoor_route_follower')
        ...
        self._pub_full_path      = self.create_publisher(Path,   'route_follower/full_path',      latched)
        self._pub_remaining_path = self.create_publisher(Path,   'route_follower/remaining_path', latched)
    ...
    def _routes_poll_cb(self):
        ...
        If active_map is still None (map detection failed at startup),
        retry _detect_active_map() here instead of silently returning.
        This means the node self-heals — no restart needed if
        lidar_localization wasn't ready when the node launched.
    ...
    def _start_cb(self, _msg: Empty):
        if self.active_map is None:
            self.get_logger().error(
                ...
                'Is lidar_localization running? Will retry map detection automatically.'
            )
            return
    ...
    def _publish_available_routes(self):
        ...
        Format:
          { "map": "adibatla_outdoor", "routes": ["patrol_zone_a", ...] }
    ...
    def _load_yaml(self) -> dict:
        """Load outdoor_waypoints.yaml. Returns {} if missing or unparseable."""
    ...
    def _load_poses(self) -> list:
        ...
        only the active map's copy is used.
        """
        if self.active_map is None:
            self.get_logger().error(
        ...
        if 'maps' not in data or not isinstance(data.get('maps'), dict):
            self.get_logger().error(
                f'outdoor_waypoints.yaml has no "maps" key. '
                f'Record some routes first with outdoor_waypoint_recorder.'
            )
            return []
    ...
    def _run(self, is_resume: bool):
        ...
        self.get_logger().info('OutdoorRouteFollower stopped')
    ...
    def _run_fresh(self):
        ...
        if len(base_poses) < 2:
            self.get_logger().error(
                ...
                'Check map name, route name, and that outdoor_waypoints.yaml exists.'
            )
            with self._state_lock:
                self._state = State.IDLE
            return
